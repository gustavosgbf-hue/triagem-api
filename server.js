...
// ── LIMPEZA AUTOMÁTICA — atendimentos travados ────────────────────────
// Expira atendimentos ‘assumido’ sem encerramento após 48h
setInterval(async () => {
try {
const result = await pool.query(`UPDATE fila_atendimentos SET status = 'expirado', encerrado_em = NOW() WHERE status = 'assumido' AND assumido_em < NOW() - INTERVAL '48 hours' RETURNING id, nome`);
if (result.rowCount > 0) {
console.log(`[LIMPEZA] ${result.rowCount} atendimento(s) expirado(s):`, result.rows.map(r => `#${r.id} ${r.nome}`).join(’, ’));
}
} catch(e) {
console.error(”[LIMPEZA] Erro ao expirar atendimentos:”, e.message);
}
...
try {
await pool.query(`CREATE TABLE IF NOT EXISTS mensagens ( id SERIAL PRIMARY KEY, atendimento_id INTEGER NOT NULL, autor TEXT NOT NULL, autor_id INTEGER, texto TEXT NOT NULL, criado_em TIMESTAMP DEFAULT NOW() )`);
...
`UPDATE fila_atendimentos SET triagem = $2, queixa = $3, idade = $4, sexo = $5, alergias = $6, cronicas = $7, medicacoes = $8, status = 'aguardando' WHERE id = $1 AND status = 'triagem' RETURNING id, nome, tel, cpf, tipo, triagem, tel_documentos, medico_nome`,
[
...
const result = await pool.query(
`SELECT id, atendimento_id, autor, texto, criado_em FROM mensagens WHERE atendimento_id = $1 ORDER BY criado_em ASC`,
[atendimentoId]
);
...
try {
const result = await pool.query(
`SELECT id, status, tipo, medico_nome, meet_link, criado_em, assumido_em, encerrado_em FROM fila_atendimentos WHERE id = $1`,
[req.params.id]
);
...
const result = await pool.query(
`INSERT INTO medicos (nome, nome_exibicao, email, senha_hash, crm, uf, telefone, especialidade, status_online, ativo, role, status) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, false, false, 'medico', 'pendente') RETURNING id, nome, nome_exibicao, email`,
...
try {
const result = await pool.query(
`SELECT id, nome, tel, cpf, tipo, triagem, status, medico_id, medico_nome, meet_link, criado_em FROM fila_atendimentos WHERE status IN ('aguardando', 'assumido') ORDER BY criado_em ASC`
);
...
appendToSheet(“Atendimentos”, [
agora, at.nome || “”, at.tel || “”, at.cpf || “”,
“Encerrado”, at.medico_nome || “”, at.triagem || “”, at.tipo || “”,
at.documentos_emitidos || “”, String(at.id),
...
const css = `
  *{box-sizing:border-box}
  ...
  .badge.expirado{background:rgba(255,100,100,.1);color:#ff6464}
...
const html = `<html><head><meta charset="utf-8"><style>body{font-family:'Segoe UI',sans-serif;background:#060d0b;color:#fff;padding:32px;max-width:800px;margin:0 auto}h1{color:#b4e05a}table{width:100%;border-collapse:collapse}th{text-align:left;padding:10px 12px;background:rgba(180,224,90,.1);color:#b4e05a;font-size:.8rem;text-transform:uppercase}td{padding:10px 12px;border-bottom:1px solid rgba(255,255,255,.06);font-size:.88rem}.badge{display:inline-block;padding:3px 10px;border-radius:999px;background:rgba(94,224,160,.1);color:#5ee0a0;font-size:.75rem}a{color:#5ee0a0}.back{display:inline-block;margin-bottom:20px;color:#b4e05a;font-size:.82rem}</style></head><body> <a class="back" href="/relatorio${senhaParam}">← Voltar ao relatório</a> <h1>✅ Consentimentos LGPD</h1><p style="color:rgba(255,255,255,.4)">${lista.length} aceites</p> <table><tr><th>Data</th><th>Nome</th><th>WhatsApp</th><th>Versão</th><th>IP</th></tr> ${lista.map(i => `<tr><td style="color:rgba(255,255,255,.45);font-size:.8rem">${i.data||’—’}</td><td>${i.nome||’—’}</td><td><a href="https://wa.me/55${String(i.tel||'').replace(/\D/g,'')}">📱 ${i.tel||’—’}</a></td><td><span class="badge">${i.versao||‘v1.0’}</span></td><td style="color:rgba(255,255,255,.3);font-size:.75rem">${i.ip||’—’}</td></tr>`).join('')} </table></body></html>`;
res.send(html);
} catch (e) { res.status(500).send(“Erro: “ + e.message); }
});
...
res.json({
  ok: true, disponivel, medicosOnline, pacientesAguardando,
  tempoEstimado, status, mensagem, horarioRetorno, horariosAgendamento,
  horaAtual: hora, horaInicio: HORA_INICIO, horaFim: HORA_FIM
});
```
...
const result = await pool.query(
`INSERT INTO agendamentos (nome, tel, tel_documentos, cpf, modalidade, horario_agendado, status) VALUES ($1, $2, $3, $4, $5, $6, 'pendente') RETURNING id`,
...
try {
const result = await pool.query(
`SELECT id, nome, tel, tel_documentos, cpf, modalidade, horario_agendado, status, criado_em FROM agendamentos WHERE status = 'confirmado' ORDER BY horario_agendado ASC`
);
