    ...
    <div id="agend-section" style="display:none">
      ...
      <div class="box" style="border-radius:18px;text-align:left">
        ...
        <div style="padding:18px 22px 22px">
          ...
          <div id="email-agend-wrap" style="margin-top:16px;display:block">
            <label style="font-size:.72rem;font-weight:600;letter-spacing:.1em;text-transform:uppercase;color:rgba(255,255,255,.35);display:block;margin-bottom:8px">Seu e-mail <span style="color:rgba(255,255,255,.5);text-transform:none;letter-spacing:0;font-weight:400">(para confirmação e lembrete)</span></label>
            <input id="inputEmailAgend" type="email" placeholder="seu@email.com" style="width:100%;padding:11px 14px;border-radius:10px;border:1px solid rgba(255,255,255,.15);background:rgba(255,255,255,.06);color:#fff;font-family:Outfit,sans-serif;font-size:.88rem;outline:none;box-sizing:border-box;display:block" oninput="validarEmailAgend()">
            <div id="emailAgendErro" style="font-size:.72rem;color:#ff8080;margin-top:4px;display:none">E-mail inválido</div>
          </div>
...
var pacienteNome = '', pacienteTel = '', pacienteTelDoc = '', pacienteCPF = '', pacienteNascimento = '', pacienteEmail = '', tipoConsulta = '';
...
function validarEmailAgend(){
  var v = (document.getElementById('inputEmailAgend')||{}).value||'';
  var ok = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v.trim());
  var erroEl = document.getElementById('emailAgendErro');
  if(erroEl) erroEl.style.display = v&&!ok?'block':'none';
  var btn=document.getElementById('btnAgendar');
  if(horarioAgendado && ok){ btn.disabled=false; btn.classList.add('on'); }
  else{ btn.disabled=true; btn.classList.remove('on'); }
}
...
function prosseguirAgendamento(){
  if(!horarioAgendado){ alert('Selecione um horário.'); return; }
  var emailEl = document.getElementById('inputEmailAgend');
  var email = emailEl ? emailEl.value.trim() : '';
  if(!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){ alert('Informe um e-mail válido para receber a confirmação.'); return; }
  pacienteEmail = email;
  modoAgendamento=true; irParaPagamento(true);
}
...
async function confirmarAgendamento(nome,tel,cpf){
  try{
    var r1=await fetch(API+'/api/agendamento/criar',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({nome,tel,tel_documentos:pacienteTelDoc,cpf,modalidade:tipoConsulta,horario_agendado:horarioAgendado,email:pacienteEmail})});
    ...
    // Cria pré-registro na fila para ter atendimentoId antes da triagem
    try{
      var rPre=await fetch(API+'/api/notify',{method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({nome,tel,tel_documentos:pacienteTelDoc,cpf,
          triagem:'(Aguardando triagem de agendamento)',tipo:tipoConsulta,data_nascimento:pacienteNascimento})});
      var dPre=await rPre.json();
      if(dPre.atendimentoId){
        atendimentoId=dPre.atendimentoId; salvarSessao();
        if(dPre.linkRetorno) history.replaceState(null,'',dPre.linkRetorno);
      }
    }catch(e){ console.warn('Pre-registro agendamento error:',e); }
...
async function sendTriage(text){
  ...
  try{
    ...
    if(!parsed){
      var match=clean.match(/\{[\s\S]*?"done"\s*:\s*true[\s\S]*?\}/);
      if(match){ try{ parsed=JSON.parse(match[0]); }catch(e){} }
    }
    if(parsed&&parsed.done&&parsed.summary){
  ...
  try{
    if(atendimentoId){
      ...
      var r=await fetch(API+'/api/atendimento/atualizar-triagem',{method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({atendimentoId,triagem:anamnesis.summary||'',agendamentoId:agendamentoId||null})});
      var d=await r.json();
      if(!d.ok) throw new Error(d.error||'Erro ao registrar triagem');
    } else {
      // Sem pré-registro: cria direto com status 'aguardando'
      console.warn('[TRIAGEM] Sem atendimentoId — criando via /api/notify direto');
